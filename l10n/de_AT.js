OC.L10N.register(
    "guests",
    {
    "Loading…" : "Lade…",
    "Loaded" : "Geladen",
    "Reset" : "Zurücksetzen",
    "Error while sharing" : "Fehler beim Teilen",
    "Error" : "Fehler",
    "Add multiple users and guests" : "Mehrere Benutzer und Gäste hinzufügen",
    "Add {unknown}" : "{unknown} hinzufügen",
    "Error loading activities" : "Fehler beim Laden der Aktivitäten",
    "Access to this resource is forbidden for guests." : "Der Zugriff auf diese Ressource ist für Gastnutzer untersagt. ",
    "Invalid mail address" : "Ungültige E-Mail-Adresse",
    "No such guest user" : "Kein derartiger Gast Benutzer",
    "The token is invalid" : "Der Token ist ungültig",
    "Password cannot be empty" : "Das Passwort darf nicht leer sein",
    "Group name must not be empty." : "Der Gruppenname muss angegeben werden.",
    "Saved" : "Gespeichert",
    "Domain is blocked." : "Die Domain ist blockiert",
    "A username with that email already exists." : "Ein Benutzername mit dieser E-Mail existiert bereits.",
    "A guest user can not create other guest users." : "Ein Gast-Benutzer kann keinen anderen Gast-Benutzer erstellen.",
    "User successfully created" : "Benutzer erfolgreich erstellt",
    "%s shared »%s« with you" : "%s hat „%s“ mit Dir geteilt",
    "%s via %s" : "%s durch %s",
    "Couldn't send reset email. Please contact your administrator." : "Die E-Mail zum Zurücksetzen konnte nicht versendet werden. Bitte kontaktiere Deinen Administrator.",
    "%s invited you" : "%s hat Dich eingeladen",
    "Email" : "E-Mail",
    "Password" : "Passwort",
    "Set password" : "Passwort setzen",
    "Hey there,\n\njust letting you know that %s shared %s with you.\n\nActivate your guest account at %s by setting a password: %s\n\nThen view it: %s\n\nYou can login using the email address \"%s\".\n\n" : "Hallo,\n\n%s hat gerade %s mit dir geteilt.\n\nAktiviere dein Gastkonto %s indem du ein Passwort vergibst: %s\n\nSieh es dir danach an: %s\n\nDu kannst dich mit deiner E-Mail Adresse einloggen \"%s\".\n\n",
    "The share will expire on %s." : "Die Freigabe wird am %s ablaufen.",
    "Cheers!" : "Noch einen schönen Tag!",
    "Hey there,<br><br>\n         \n         just letting you know that %s shared <strong>%s</strong> with you.<br><br>\n         Activate your guest account at %s by <a href=\"%s\">setting a password</a>.<br><br>\n         Then <a href=\"%s\">view it!</a><br><br>You can login using the email address <strong>\"%s\"</strong> .<br><br>" : "Hallo,<br><br>\n\n%s hat gerade <strong>%s</strong> mit dir geteilt.<br><br>\nAktiviere dein Gastkonto auf %s indem Du <a href=\"%s\">ein Passwort vergibst</a>.<br><br>\n<a href=\"%s\">Danach kannst Du es dir ansehen!</a><br><br>\nDu kannst dich mit deiner E-Mail Adresse <strong>\"%s\"</strong> einloggen.<br><br>",
    "Hey there,<br><br>\n         \n         just letting you know that %s shared files with you.<br><br>\n         Activate your guest account at %s by <a href=\"%s\">setting a password</a>.<br><br>\n         <br><br>You can login using the email address <strong>\"%s\"</strong> .<br><br>" : "Hallo,<br><br>\n         \n         zu Deiner Info %s hat Dateien mit Dir geteilt.<br><br>\n         Aktiviere hier %s Deinen Gast Account durch <a href=\"%s\">setzen eines Passwortes</a>.<br><br>\n         <br><br>Du kannst Dich mit dieser Email Adresse einloggen <strong>\"%s\"</strong> .<br><br>",
    "Guest users are grouped under a virtual group in the user manager" : "Gäste sind unter einer virtuellen Gruppe im  Nutzermanager zusammengefasst",
    "Group name" : "Gruppenname",
    "Domain blocklist for guest user invitation" : "Diese Domain ist für Gäste Einladungen blockiert",
    "The following domains will not be accepted when users try to invite guests." : "Folgende Domains werden für Gast Einladungen nicht akzeptiert.",
    "Limit guest access to an app whitelist" : "Gastzugriff auf eine App-Whitelist beschränken",
    "Reset whitelist" : "Whitelist zurücksetzen",
    "Add guest: {email}" : "Gast hinzufügen: {email}",
    "Error while sharing. The guest user has been removed. Please check if sharing is allowed for this user." : "Fehler beim Teilen. Der Gastbenutzer wurde entfernt. Bitte prüfen Sie, ob das Teilen für diesen Benutzer erlaubt ist.",
    "User is not a guest user." : "Benutzer ist kein Gastbenutzer.",
    "Cannot remove an active guest user." : "Ein aktiver Gastbenutzer kann nicht entfernt werden.",
    "Guest user was created too long ago to be removed automatically." : "Der Gastbenutzer wurde vor zu langer Zeit erstellt, um automatisch entfernt zu werden.",
    "User not found." : "Benutzer nicht gefunden.",
    "Guest user removed." : "Gastbenutzer wurde entfernt."
},
"nplurals=2; plural=(n != 1);");
