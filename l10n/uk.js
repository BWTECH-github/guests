OC.L10N.register(
    "guests",
    {
    "Loading…" : "Завантаження…",
    "Loaded" : "Завантажено",
    "Reset" : "Перевстановити",
    "Error while sharing" : "Помилка під час публікації",
    "Error" : "Помилка",
    "Error loading activities" : "Помилка завантаження подій",
    "Access to this resource is forbidden for guests." : "Доступ до цього ресурсу для гостів відсутній.",
    "Invalid mail address" : "Неправильна email адреса",
    "No such guest user" : "Немає такого користувача-гостя",
    "The token is invalid" : "Ключ недійсний",
    "Password cannot be empty" : "Пароль не може бути порожнім",
    "Group name must not be empty." : "Ім'я групи не може бути порожнім.",
    "Saved" : "Збережено",
    "A username with that email already exists." : "Користувач із такою адресою email вже існує.",
    "User successfully created" : "Користувача успішно створено",
    "%s shared »%s« with you" : "%s поділився з вами »%s«",
    "%s via %s" : "%s за допомогою %s",
    "Couldn't send reset email. Please contact your administrator." : "Не вдалося надіслати email для скидання. Будь ласка, зверніться до вашого адміністратора.",
    "Email" : "E-mail",
    "Password" : "Пароль",
    "Set password" : "Встановіть пароль",
    "Hey there,\n\njust letting you know that %s shared %s with you.\n\nActivate your guest account at %s by setting a password: %s\n\nThen view it: %s\n\nYou can login using the email address \"%s\".\n\n" : "Агов,\n\nповідомляю, що %s поділився з Вами %s.\n\nАктивуйте ваш обліковий запис гостя на %s встановивши пароль: %s\n\nІ потім перегляньте: %s\n\nВи можете увійти використовуючи адресу email \"%s\".\n\n",
    "The share will expire on %s." : "Спільний доступ закінчиться %s.",
    "Cheers!" : "Будьмо!",
    "Hey there,<br><br>\n         \n         just letting you know that %s shared <strong>%s</strong> with you.<br><br>\n         Activate your guest account at %s by <a href=\"%s\">setting a password</a>.<br><br>\n         Then <a href=\"%s\">view it!</a><br><br>You can login using the email address <strong>\"%s\"</strong> .<br><br>" : "Агов,<br><br>\n         \n         повідомляю, що %s поділився з Вами <strong>%s</strong>.<br><br>\n         Активуйте ваш обліковий запис гостя %s <a href=\"%s\">встановивши пароль</a>.<br><br>\n         І <a href=\"%s\">перегляньте опубліковане</a>.<br><br>Ви можете увійти використовуючи адресу email <strong>\"%s\"</strong> .<br><br>",
    "Guest users are grouped under a virtual group in the user manager" : "Користувачі-гості перебувають у віртуальній групі в панелі управління користувачами",
    "Group name" : "Ім'я групи",
    "Limit guest access to an app whitelist" : "Обмежити доступ гостів до додатку білим списком",
    "Reset whitelist" : "Очистити білий список",
    "Add guest: {email}" : "Додати гостя: {email}",
    "Error while sharing. The guest user has been removed. Please check if sharing is allowed for this user." : "Помилка під час спільного доступу. Гостьового користувача видалено. Будь ласка, перевірте, чи дозволено спільний доступ для цього користувача.",
    "User is not a guest user." : "Користувач не є гостьовим.",
    "Cannot remove an active guest user." : "Неможливо видалити активного гостьового користувача.",
    "Guest user was created too long ago to be removed automatically." : "Гостьовий користувач був створений занадто давно для автоматичного видалення.",
    "User not found." : "Користувача не знайдено.",
    "Guest user removed." : "Гостьового користувача видалено."
},
"nplurals=4; plural=(n % 1 == 0 && n % 10 == 1 && n % 100 != 11 ? 0 : n % 1 == 0 && n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 12 || n % 100 > 14) ? 1 : n % 1 == 0 && (n % 10 ==0 || (n % 10 >=5 && n % 10 <=9) || (n % 100 >=11 && n % 100 <=14 )) ? 2: 3);");
