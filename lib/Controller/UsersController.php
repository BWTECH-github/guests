...
class UsersController extends Controller {
	 ...
	 * @NoAdminRequired
	 *
	 * Delete a guest user (rollback after failed share creation).
	 * Only allows deletion of guest users that were recently created
	 * and have not yet set a password.
	 *
	 * @param string $username
	 * @return DataResponse
	 */
	public function delete(string $username): DataResponse {
		$username = \strtolower(\trim($username));

		// Verify current user is logged in
		$currentUserObj = $this->currentUser->getUser();
		if ($currentUserObj === null) {
			return new DataResponse(
				['message' => (string)$this->l10n->t('No user logged in.')],
				Http::STATUS_FORBIDDEN
			);
		}
		...
		if ($isGuest !== '1') {
			return new DataResponse(
				['message' => (string)$this->l10n->t('User is not a guest user.')],
				Http::STATUS_FORBIDDEN
			);
		}

		// Only allow deletion if the guest still has a register token
		// (meaning they haven't set a password yet)
		...
		if ($registerToken === '') {
			return new DataResponse(
				['message' => (string)$this->l10n->t('Cannot remove an active guest user.')],
				Http::STATUS_FORBIDDEN
			);
		}

		// Only allow deletion of recently created guests (within 5 minutes)
		$createdTime = $this->config->getUserValue(
			$username,
			'guests',
			'created',
			'0'
		...
		$timeDiff = \time() - (int)$createdTime;
		if ($timeDiff > 300) {
			return new DataResponse(
				['message' => (string)$this->l10n->t('Guest user was created too long ago to be removed automatically.')],
				Http::STATUS_FORBIDDEN
			);
		}
		...
		if ($user === null) {
			return new DataResponse(
				['message' => (string)$this->l10n->t('User not found.')],
		...
		return new DataResponse(
			['message' => (string)$this->l10n->t('Guest user removed.')],
			Http::STATUS_OK
		);
	}
